...
class Character{
  ...
  getInfo(name, type){
    return 'This is a ${this.type} character named ${name}'
  }
}
...
let ralphsLocation = ralph.this.location
...
class Player extends Character{
  ...
  defend(attackLevel, healthLevel){
    if(healthLevel > 0)
    return {attackStrength: this.attackLevel,
            remainingHealth: this.healthLevel,
            message: '${this.name} is still in the fight!'
    }
    else return "${name} has been defeated"
  }
}
...
fireSpitter.addSuperPower('talking')

let fireSpitterAttack = Hero.useSuperPower(0)